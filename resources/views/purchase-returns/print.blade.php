...
<head>
    <meta charset="utf-8">
    <title>Purchase Returns — {{ $setting->company_name ?? 'Alphainno POS' }}</title>
    <style>
...
<div class="toolbar">
    ...
    <a href="{{ route('purchase-returns.index', request()->only(['from','to'])) }}" class="btn">Back</a>
</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $r->purchase?->reference ?? '—' }}</td>
                <td>{{ $r->supplier?->name ?? '—' }}</td>
                <td class="r">{{ $fmt($r->total) }}</td>
                <td>{{ $r->notes ?: '—' }}</td>
            </tr>
            @empty
            <tr><td colspan="6">No returns in this period.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
