...
@section('content')
@php $openCreate = request('create') === '1' || $errors->any(); @endphp

<div x-data="{ showCreate: @json($openCreate) }">
    ...
    {{-- Create Supplier — right slide panel --}}
    <div x-show="showCreate" x-cloak class="fixed inset-0 z-[250]" @keydown.escape.window="showCreate = false">
        ...
        <div class="absolute top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
                ...
                <h3 class="font-bold text-slate-900 flex-1 text-center pr-6">Create Supplier</h3>
            </div>

            <div class="flex-1 overflow-y-auto p-5">
                <h4 class="text-sm font-semibold text-slate-800 mb-3">Add Supplier</h4>
                @if ($errors->any())
                    <div class="mb-3 rounded-lg bg-red-50 border border-red-200 text-red-700 px-3 py-2 text-sm">
                ...
                <form method="POST" action="{{ route('suppliers.store') }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <input name="name" value="{{ old('name') }}" required class="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-ai-purple focus:ring-2 focus:ring-ai-purple/15 outline-none">
                    </div>
                    <div>
                        <label class="text-sm font-medium text-slate-700">Phone <span class="text-red-500">*</span></label>
                        <input name="phone" value="{{ old('phone') }}" required class="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-ai-purple focus:ring-2 focus:ring-ai-purple/15 outline-none">
                    </div>
                    <div>
                        <label class="text-sm font-medium text-slate-700">Email</label>
                        <input type="email" name="email" value="{{ old('email') }}" class="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-ai-purple focus:ring-2 focus:ring-ai-purple/15 outline-none">
                    </div>
                    <div>
                        <label class="text-sm font-medium text-slate-700">Address</label>
                        <input name="address" value="{{ old('address') }}" class="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-ai-purple focus:ring-2 focus:ring-ai-purple/15 outline-none">
                    </div>
                ...
                <div class="mt-6 pt-6 border-t border-slate-100">
                    ...
                    <form method="POST" action="{{ route('suppliers.import') }}" enctype="multipart/form-data" class="space-y-3" id="supplier-csv-form">
                        @csrf
                        <input type="file" name="csv_file" accept=".csv,.txt" id="supplier-csv-input" class="w-full text-sm text-slate-600 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-slate-100 file:text-slate-700">
                        <button type="submit" id="supplier-csv-btn" disabled class="w-full py-2.5 rounded-lg bg-slate-300 text-white font-medium text-sm cursor-not-allowed">Import From CSV</button>
                    </form>
                </div>
            </div>
        </div>
...
@push('scripts')
<script>
document.getElementById('supplier-csv-input')?.addEventListener('change', function () {
    const btn = document.getElementById('supplier-csv-btn');
    if (!btn) return;
    const hasFile = this.files && this.files.length > 0;
    btn.disabled = !hasFile;
    btn.classList.toggle('bg-slate-300', !hasFile);
    btn.classList.toggle('cursor-not-allowed', !hasFile);
    btn.classList.toggle('bg-slate-500', hasFile);
    btn.classList.toggle('hover:bg-slate-600', hasFile);
});
</script>
@endpush
@endsection
